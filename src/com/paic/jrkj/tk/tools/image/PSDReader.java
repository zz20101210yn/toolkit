...
public class PSDReader {
    ...
    private final Log logger = LogFactory.getLog(PSDReader.class);
     ...
     *
     * @param stream InputStream in PhotoShop format.
     * @return int read status code (0 = no errors)
     */
    ...
    private void readLineLengths(int nLines) {
    ...
    private int readBytes(byte[] bytes, int n) {
    ...
    private byte[] readPlane(int w, int h) {
    ...
    private byte[] readPlaneCompressed(int w, int h) {
        byte[] b = new byte[w * h];
        byte[] s = new byte[w * 2];
        int pos = 0;
        for (int i = 0; i < h; i++) {
    ...
    private void decodeRLE(byte[] src, int sindex, int slen, byte[] dst, int dindex) {
        try {
            int max = sindex + slen;
            while (sindex < max) {
                byte b = src[sindex++];
